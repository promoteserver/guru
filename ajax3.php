...
$sql=mysql_query("SELECT DISTINCT e.id as event_id,eg.id as egId ,eg.photo as egphoto,e.title as title,e.introduction as introduction,e.start as start,e.modified as M from events as e left join events_gallery as eg on eg.event_id=e.id where e.start <= CURDATE() GROUP BY e.id order by M desc LIMIT ".$load.",5");
...
}//end if si los eventos ya finalizaron
