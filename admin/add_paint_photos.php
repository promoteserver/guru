...
$(document).ready(function() {
	$("#uploadify").uploadify({
		...
		'script'         : 'uploadfiles/uploadify_img.php?albid2=<?=@$_GET['albid2']?>',
...
if(isset($_GET['albid2']) and !empty($_GET['albid2'] )){
	$albidId2 = intval($_GET['albid2']);
	$sql1 = mysql_query("SELECT * FROM users WHERE id='$albidId2'");
...
<div id="interface">
  <div class="dashBoard-admin">
  	<h1>Paintings of <?php echo $row2['username'] ?> </h1>
...
Check the image format : They should be in .JPG format and the minimun dimensions are: 640px * 640px (Square Image) . Thanks!
</div>
...
<input type="file" name="uploadify" id="uploadify" />
    ...
    <?php
	     if(isset($_GET['albid2']) and !empty($_GET['albid2']) ){
			 $albId = intval($_GET['albid2']);
			 $sql = mysql_query("SELECT * FROM users_paints WHERE user_id='$albId'");
			 $cnt = mysql_num_rows($sql);
			 if($cnt > 0){
				 while($row = mysql_fetch_array($sql)){
					 echo '<div class="thumb-case">
					       <img src="../thumbnail.php?src=/images/gallery/zoom/'.$row['photo'].'&h=60&w=60&q=90" class="thubm_view" />
						   <a href="javascript:deletePicPaint('.$row['id'].')">
