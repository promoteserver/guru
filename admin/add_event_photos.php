...
$(document).ready(function() {
	$("#uploadify").uploadify({
		...
		'script'         : 'uploadfiles/uploadify_img.php?eventid=<?=@$_GET['eventid']?>',
...
if(isset($_GET['eventid']) and !empty($_GET['eventid'] )){
	$eventId = intval($_GET['eventid']);
	$sql1 = mysql_query("SELECT * FROM events WHERE id='$eventId'");
...
<div id="interface">
  <div class="dashBoard-admin">
  	<h1>Event : 	<?php echo $row2['title']; ?> </h1>
...
<input type="file" name="uploadify" id="uploadify" />
    ...
    <?php
	     if(isset($_GET['eventid']) and !empty($_GET['eventid']) ){
			 $eventId = intval($_GET['eventid']);
			 $sql = mysql_query("SELECT * FROM events_gallery WHERE event_id='$eventId'");
			 $cnt = mysql_num_rows($sql);
			 if($cnt > 0){
				 while($row = mysql_fetch_array($sql)){
					 echo '<div class="thumb-case">
					       <img src="../thumbnail.php?src=/images/gallery/zoom/'.$row['photo'].'&h=60&w=60&q=100" class="thubm_view" />
						   <a href="javascript:deletePicEvent('.$row['id'].')">
