  ...
  ?>
    ...
    <?php
	      if($cUserRole==1)
    	  {
		  	...
		  	$sql = mysql_query("SELECT * FROM  events  where user_id=$cUserId ORDER BY id DESC");

		  }
		  while($row = mysql_fetch_array($sql)){
			  echo '<tr>
	                    <td>
	                    	<a href="edit_event.php?cx='.$row['id'].'">'.$row['title'].'</a>
	                    </td>


	                    <td>
	                    	<a href="edit_event.php?cx='.$row['id'].'">'.$row['start'].'</a>
	                    </td>

						<td>
							<a href="add_event_photos.php?eventid='.$row['id'].'">
							<img src="images/icons/new_album.png" border="0" />
							</a>
							&nbsp;('.$objGal->countGaleryPicEvent($row['id']).')
						</td>';
					if($cUserRole==1) //Si es admin puede eliminar usuarios
					{

					echo'<td><a href="javascript:deleteEvent('.$row['id'].')">
					<img src="images/icons/delete.png" border="0" /></a>&nbsp;&nbsp
							
											
						<a href="edit_event.php?cx='.$row['id'].'">
